...
- [x] T1 初始化 package.json / tsconfig / vite 配置，编写 plugin.json 清单与最小入口，跑通构建
...
- [x] T2 实现 stripFence / wrapFence 纯函数并覆盖空行、无围栏、异常输入用例
...
- [x] T3 实现可取消防抖（debounce + cancel + flush），用 fake timers 验证拖拽期间零触发
...
- [x] T4 定义 RenderBackend 接口与 VisimerBackend 实现（画布初始化/销毁/变更回调），接口支持第三方后端注入
...
- [x] T5 实现 DiagramAdapter 接口、注册表与首行类型判定路由（full/readonly/未知三路）
...
- [x] T6 实现 flowchart 适配器（supportLevel=full，组装 VisimerBackend），注册入 registry
...
- [x] T7 实现 readonly 适配器（mermaid 渲染 + 禁编辑 + 未知类型提示文案），注册为兜底
...
- [x] T8 实现 Dialog 打开/关闭/销毁生命周期，容器挂载与后端 destroy 联动
...
- [x] T9 实现 block-icon 注入与点击触发（仅 mermaid 代码块），无副作用于普通块
...
- [x] T10 实现快捷键注册（默认 Shift+Alt+M）、块内判定与设置项改键持久化
...
- [x] T12 实现 isSyncing 锁 + 编辑版本号比对，防旧回调覆盖新编辑
